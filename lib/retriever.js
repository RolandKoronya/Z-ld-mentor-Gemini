// lib/retriever.js
// UPDATED: Now uses Google Gemini for embeddings with gemini-embedding-001
...
// ⬇️ UPDATED DEFAULT EMBED MODEL TO ACTIVE VALUE ⬇️
export function createRetriever(kb, { geminiApiKey, embedModel = "gemini-embedding-001" } = {}) {
  ...
  async function search(query, { k = 12, k_kw = 80, alpha = 0.55 } = {}) {
    ...
    for (const id of candidateIds) {
      ...
      // Safety check: ensure chunk has an embedding
      const semScore = (qEmb && ch.embedding) ? cosine(qEmb, ch.embedding) : 0;
